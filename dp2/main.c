...
void addintToStr(char * list, int inputVal){ 
	char buffer[10]; 
...
int findGrouping(int array_input[], int begining, int ending)
{
	...
	if(0) printf("Testing Grouping of length %d\n", lengthleft);
	int copy_array[begining];
	int kk;
	if (lengthleft < 1)
	{
	if(0)printf("Trivial Solution of %d \n", kk, array_input[begining]);
	...
	if(0) printArray(copy_array,begining);
	bool completeList = true;
	for (kk = 0 ; kk < begining-1 ; kk++)
	{
	if( (copy_array[kk]+1) != copy_array[kk+1])
	{
	if(0)printf("at %d, %d Doesn't Match %d\n", kk, (copy_array[kk]+1), copy_array[kk+1]);
	completeList = false;
	break;
	}
	}
	if (completeList)
	{
	if(0)printf("Found a Group up to %d\n", ending-begining);
	return begining;
	}
	else
	{
	if(0)printf("Didn't Find a Group up to %d\n", begining);
...
void findNextFlip(int array_input[], int array_sort[], int curSearchInt, char * returnString, int curMinLoc, int curMaxLoc)
{
	...
	// ---------------------- Take care of the Unknown Actions-----------------------------------------	
	...
	int betterFlipAt = findGrouping(array_input, kk, curMaxLoc);
	...
	{
	printf("Found Smaller Grouping");
	printArray(array_input, curSearchInt+1);
	
	int curFlipSetMin = 2*curMaxLoc;
	int kkk;
	for (kkk = 0; kkk < kk+betterFlipAt; kkk++)
	{
	if(array_input[kkk] < curFlipSetMin) {curFlipSetMin = array_input[kkk];}
	}
	printArray(array_input,curSearchInt+1);
	printf("found Min at: %d\n", curFlipSetMin);
...
int main(void)
{
	...
	//int array_to_Organize[] = {2, 6, 5, 4, 1, 7, 3 };
	//int array_to_Organize[] = {3, 2, 4, 1, 6, 7, 5};
	//int array_to_Organize[] = {1, 2, 3, 4,};
	//int array_to_Organize[] = {5, 4, 3, 2, 1,};
	//int array_to_Organize[] = {4, 5, 6, 3, 2, 1};
	...
	findGrouping(array_to_Organize, 5, n);
	...
	findNextFlip(array_to_Organize, sorted_array ,n-1, returnedString, curMinLoc, curMaxLoc);
...
On Mon, Oct 17, 2016 at 3:32 PM, Lorin Hendricks <maxwellhendricks@gmail.com> wrote:
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdbool.h>
...
On Mon, Oct 17, 2016 at 12:23 AM, Lorin Hendricks <maxwellhendricks@gmail.com> wrote:
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdbool.h>
...
int findGrouping(int array_input[], int begining, int ending)
{
	...
	{
	copy_array[kk] = array_input[kk+begining];
	}
	qsort(copy_array, length, sizeof(int), cmpfunc);
	
	if(1) printArray(copy_array,length);
	bool completeList = true;
	...
	return findGrouping(array_input, begining, ending-1);
	}
}
...
int main(void)
{
	...
	findGrouping(array_to_Organize, 1, n);
